...
def _dirname(p):
    ...
    """
    # See D74424098 summary for why we use indexing with rfind rather than the
    # rpartition function - tldr, it allocates much less memory and this
    # function is used heavily in our large build graphs.
    index = p.rfind("/")
    if index == -1:
        return ""
    else:
