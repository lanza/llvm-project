  ...
  mlir::Value buildScalarPrePostIncDec(const UnaryOperator *E, LValue LV,
    ...
    if (isInc && type->isBooleanType()) {
      ...
      if (CGF.getContext().isPromotableIntegerType(type)) {
        ...
        auto promotedCirTy = mlir::dyn_cast<mlir::cir::IntType>(ConvertType(promotedType));
...
#undef HANDLEBINOP
  ...
  buildScalarConversion(mlir::Value Src, QualType SrcType, QualType DstType,
    ...
    assert(!mlir::isa<::mlir::cir::PointerType>(SrcTy) &&
           !mlir::isa<::mlir::cir::PointerType>(DstTy) &&
...
mlir::Value ScalarExprEmitter::buildScalarCast(
  ...
  } else if (mlir::isa<mlir::FloatType>(SrcTy)) {
    if (CGF.getBuilder().isInt(DstTy)) {
