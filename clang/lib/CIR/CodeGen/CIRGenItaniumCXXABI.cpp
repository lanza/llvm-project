...
void CIRGenItaniumCXXABI::buildGuardedInit(CIRGenFunction &cgf,
  ...
  // TODO(CIR): block name?
  mlir::Block *endBlock = cgf.createBasicBlock("init.end");
  if (!threadsafe || maxInlineWidthInbits) {
    ...
    mlir::Value needsInit = builder.createIsNull(value, "guard.uninitialized");

    mlir::Block *initCheckBlock = cgf.createBasicBlock("init.check");
  ...
  cgf.buildCXXGlobalVarDeclInit(varDecl, globalOp, performInit);
