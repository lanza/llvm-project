...
/// TODO(cir): this can unified with LLVM codegen
static bool ContainsIncompleteClassType(CIRGenModule &CGM, QualType Ty) {
  if (const RecordType *RecordTy = dyn_cast<RecordType>(Ty)) {
  ...
  if (const PointerType *PointerTy = dyn_cast<PointerType>(Ty))
    return ContainsIncompleteClassType(CGM, PointerTy->getPointeeType());
  ...
  if (const MemberPointerType *MemberPointerTy =
    ...
    return ContainsIncompleteClassType(CGM, MemberPointerTy->getPointeeType());
  }
...
static cir::GlobalLinkageKind getTypeInfoLinkage(CIRGenModule &CGM,
  ...
  if (ContainsIncompleteClassType(CGM, Ty))
...
static unsigned ComputeVMIClassTypeInfoFlags(const CXXBaseSpecifier *Base,
  ...
  auto *BaseDecl = Base->getType()->castAs<RecordType>()->getAsCXXRecordDecl();
...
CIRGenItaniumCXXABI::emitMemberPointer(const APValue &memberPointer,
  ...
  // Data member pointer.
  // If there's an adjustment, we need to handle it. Currently guarded by
  // isMemberPointerToDerivedMember check above.
  assert(thisAdjustment.isZero() &&
         "NYI: data member pointers with non-zero adjustment");
  auto &builder = CGM.getBuilder();
  auto ty = mlir::cast<cir::DataMemberType>(CGM.convertType(mpType));
