...
RValue CIRGenFunction::emitCXXMemberCallExpr(const CXXMemberCallExpr *CE,
  ...
  NestedNameSpecifier Qualifier =
      HasQualifier ? ME->getQualifier() : NestedNameSpecifier();
  NestedNameSpecifier *QualifierPtr = HasQualifier ? &Qualifier : nullptr;
  bool IsArrow = ME->isArrow();
  const Expr *Base = ME->getBase();

  return emitCXXMemberOrOperatorMemberCallExpr(
      CE, MD, ReturnValue, HasQualifier, QualifierPtr, IsArrow, Base);
}
