...
public:
  ...
  matchAndRewrite(mlir::cir::TernaryOp op, OpAdaptor adaptor,
    ...
    // TODO: This is a simplified lowering that doesn't properly handle the regions
    // A proper implementation would need to inline the regions and extract the yielded values
    return mlir::LogicalResult::failure();
  }
};
...
public:
  ...
  matchAndRewrite(mlir::cir::IfOp op, OpAdaptor adaptor,
    ...
    ifOp = rewriter.create<mlir::scf::IfOp>(op.getLoc(),
...
public:
  ...
  matchAndRewrite(mlir::cir::GlobalOp op, OpAdaptor adaptor,
    ...
    auto newGlobal = rewriter.create<mlir::memref::GlobalOp>(
        ...
        op.getInitialValueAttr(), static_cast<bool>(op.getConstantAttr()), nullptr);
