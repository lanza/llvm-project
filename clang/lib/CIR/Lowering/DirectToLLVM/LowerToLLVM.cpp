...
mlir::LogicalResult CIRToLLVMGlobalOpLowering::lowerInitializer(
  ...
  if (auto zeroAttr = mlir::dyn_cast<cir::ZeroAttr>(init)) {
...
mlir::LogicalResult CIRToLLVMGlobalOpLowering::lowerInitializerDirect(
  ...
  useInitializerRegion = false;
  // If initRewriter returned a null attribute, init will have a value but the
  // value will be null. If that happens, initRewriter didn't handle the
...
mlir::LogicalResult CIRToLLVMGlobalOpLowering::matchAndRewrite(
  ...
  if (op.getComdat()) {
    ...
    addComdat(llvmGlobalOp, comdatOp, rewriter, module);
  }
  ...
  if (name == "masked.load") {
    ...
    if (operands.size() == 4) {
      if (auto ptrTy = mlir::dyn_cast<mlir::LLVM::LLVMPointerType>(
        ...
        mlir::IntegerAttr alignAttr;
        if (auto constOp =
                operands[1].getDefiningOp<mlir::LLVM::ConstantOp>()) {
          if (auto intAttr =
                  mlir::dyn_cast<mlir::IntegerAttr>(constOp.getValue()))
            alignAttr = intAttr;
        }
        if (alignAttr) {
          auto alignment =
              rewriter.getI64IntegerAttr(alignAttr.getValue().getZExtValue());
          mlir::DictionaryAttr ptrAttr = rewriter.getDictionaryAttr(
              {rewriter.getNamedAttr("llvm.elementtype", elementTyAttr),
               rewriter.getNamedAttr("llvm.align", alignment)});
          auto emptyDict = rewriter.getDictionaryAttr({});
          mlir::ArrayAttr argAttrs =
              rewriter.getArrayAttr({ptrAttr, emptyDict, emptyDict});
...
mlir::LogicalResult CIRToLLVMBrOpLowering::matchAndRewrite(
    ...
    mlir::ConversionPatternRewriter &rewriter) const {
  auto convertedDest =
      convertSuccessorBlock(rewriter, getTypeConverter(), op, op.getDest(),
...
mlir::LogicalResult CIRToLLVMIndirectBrOpLowering::matchAndRewrite(
  ...
  for (auto operand : op.getSuccOperands()) {
