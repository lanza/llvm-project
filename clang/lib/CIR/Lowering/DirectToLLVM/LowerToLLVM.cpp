...
mlir::Value lowerCirAttrAsValue(mlir::Operation *parentOp,
  ...
  else if (auto strAttr = mlir::dyn_cast<mlir::StringAttr>(constArr.getElts())) {
...
public:
  ...
  matchAndRewrite(mlir::cir::LoopOp loopOp, OpAdaptor adaptor,
    ...
    auto bodyYield =
        mlir::dyn_cast<mlir::cir::YieldOp>(bodyRegion.back().getTerminator());
...
public:
  ...
  matchAndRewrite(mlir::cir::CastOp castOp, OpAdaptor adaptor,
    ...
    case mlir::cir::CastKind::integral: {
      auto dstType = mlir::cast<mlir::cir::IntType>(castOp.getResult().getType());
      auto srcType = mlir::dyn_cast<mlir::cir::IntType>(castOp.getSrc().getType());
      auto llvmSrcVal = adaptor.getOperands().front();
      auto llvmDstTy =
          mlir::cast<mlir::IntegerType>(getTypeConverter()->convertType(dstType));
    ...
    case mlir::cir::CastKind::floating: {
      auto dstTy = mlir::cast<mlir::FloatType>(castOp.getResult().getType());
      auto srcTy = castOp.getSrc().getType();
      auto llvmSrcVal = adaptor.getOperands().front();

      if (auto fpSrcTy = mlir::dyn_cast<mlir::FloatType>(srcTy)) {
    ...
    case mlir::cir::CastKind::int_to_float: {
      ...
      auto llvmSrcVal = adaptor.getOperands().front();
      auto llvmDstTy = getTypeConverter()->convertType(dstTy);
      if (mlir::cast<mlir::cir::IntType>(castOp.getSrc().getType()).isSigned())
    ...
    case mlir::cir::CastKind::float_to_int: {
      ...
      auto llvmSrcVal = adaptor.getOperands().front();
      auto llvmDstTy = getTypeConverter()->convertType(dstTy);
      if (mlir::cast<mlir::cir::IntType>(castOp.getResult().getType()).isSigned())
...
lowerConstArrayAttr(mlir::cir::ConstArrayAttr constArr,
  ...
  auto cirArrayType = mlir::dyn_cast<mlir::cir::ArrayType>(typedConstArr.getType());
...
public:
  ...
  matchAndRewrite(mlir::cir::FuncOp op, OpAdaptor adaptor,
    ...
    assert(mlir::isa<mlir::FileLineColLoc>(Loc) && "expected single location here");
...
public:
  ...
  matchAndRewrite(mlir::cir::SwitchOp op, OpAdaptor adaptor,
    ...
    for (size_t i = 0; i < op.getCases()->size(); ++i) {
      ...
      auto caseAttr = mlir::cast<mlir::cir::CaseAttr>(op.getCases()->getValue()[i]);
      ...
      if (caseAttr.getKind().getValue() == mlir::cir::CaseOpKind::Default) {
        ...
        for (auto &value : caseAttr.getValue()) {
          caseValues.push_back(mlir::cast<mlir::cir::IntAttr>(value).getValue());
...
public:
  ...
  matchAndRewrite(mlir::cir::GlobalOp op, OpAdaptor adaptor,
    ...
    } else if (mlir::isa<mlir::FloatAttr>(init.value())) {
...
public:
  ...
  matchAndRewrite(mlir::cir::BinOp op, OpAdaptor adaptor,
    ...
    assert((mlir::isa<mlir::cir::IntType, mlir::FloatType>(type)) &&
  ...
  for (auto namedAttr : module->getAttrs()) {
    if (namedAttr.getName() == "cir.globalCtors") {
      for (auto attr : mlir::cast<mlir::ArrayAttr>(namedAttr.getValue())) {
        ...
        if (auto ctorAttr = mlir::cast<mlir::cir::GlobalCtorAttr>(attr)) {
