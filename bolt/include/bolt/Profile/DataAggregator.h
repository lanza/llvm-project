...
private:
  ...
  ErrorOr<AggregatedLBREntry> parsePreAggregatedLBREntry();

  ErrorOr<std::vector<DataAggregator::AggregatedLBREntry>>
  parseUnsymbolizedProfileLBREntry();
  ...
  void parseUnsymbolizedProfile();

  /// Parse the full output of pre-aggregated LBR samples generated by
  /// an external tool.
  std::error_code parsePreAggregatedLBRSamples();

  /// Parse the full output of unsymbolized-profile LBR samples generated by
  /// simpleperf's branch-list format or perf2bolt's --autofdo option.
  std::error_code parseUnsymbolizedProfileLBRSamples();

  /// Process LBRAggregatedSamples from pre-aggregated or unsymbolized profiles
  void processLBRAggregatedSamples();
